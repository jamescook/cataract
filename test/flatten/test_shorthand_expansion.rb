...
# Test CSS shorthand/longhand behavior through flatten operations
# Verifies that flatten correctly expands shorthands when cascading,
# and recreates optimized shorthands in the final output
class TestShorthandExpansion < Minitest::Test
  # Helper to parse CSS and flatten rules
  def parse_and_flatten(css)
    ...
    return Cataract::Declarations.new([]) if flattened.rules_count.zero?
  ...
  def test_margin_shorthand_overrides_longhand
    ...
    decls = parse_and_flatten('.test { margin-top: 5px; margin: 10px }')
  ...
  def test_margin_longhand_overrides_shorthand
    ...
    decls = parse_and_flatten('.test { margin: 10px; margin-top: 20px }')
  ...
  def test_margin_partial_override
    # Override two sides
    decls = parse_and_flatten('.test { margin: 10px; margin-top: 20px; margin-bottom: 30px }')
  ...
  def test_margin_all_different_sides
    ...
    decls = parse_and_flatten('.test { margin-top: 1px; margin-right: 2px; margin-bottom: 3px; margin-left: 4px }')
  ...
  def test_margin_important_prevents_override
    ...
    decls = parse_and_flatten('.test { margin-top: 5px !important; margin: 10px }')
  ...
  def test_padding_longhand_overrides_shorthand
    decls = parse_and_flatten('.test { padding: 5px; padding-left: 10px }')

    assert_equal '5px 5px 5px 10px', decls['padding']
  end

  def test_padding_all_same
    decls = parse_and_flatten('.test { padding-top: 8px; padding-right: 8px; padding-bottom: 8px; padding-left: 8px }')
  ...
  def test_border_width_override
    decls = parse_and_flatten('.test { border: 1px solid red; border-top-width: 3px }')
  ...
  def test_border_all_sides_same
    decls = parse_and_flatten('.test { border-top: 2px dashed blue; border-right: 2px dashed blue; border-bottom: 2px dashed blue; border-left: 2px dashed blue }')
  ...
  def test_border_mixed_sides
    decls = parse_and_flatten('.test { border-top: 1px solid red; border-bottom: 2px dotted blue }')
  ...
  def test_font_override_family
    decls = parse_and_flatten('.test { font: 12px Arial; font-family: Helvetica }')
  ...
  def test_font_incomplete_properties
    ...
    decls = parse_and_flatten('.test { font-size: 14px; font-weight: bold }')
  ...
  def test_list_style_override
    decls = parse_and_flatten('.test { list-style: square inside; list-style-type: circle }')
  ...
  def test_list_style_single_property
    ...
    decls = parse_and_flatten('.test { list-style-type: disc }')
  ...
  def test_background_override
    decls = parse_and_flatten('.test { background: red; background-image: url(img.png) }')
  ...
  def test_background_single_property
    # Single property should not create shorthand
    decls = parse_and_flatten('.test { background-color: blue }')
  ...
  def test_background_linear_gradient
    ...
    decls = parse_and_flatten('.test { background: linear-gradient(180deg, #fff, #000) }')
  ...
  def test_background_radial_gradient
    ...
    decls = parse_and_flatten('.test { background: radial-gradient(circle, red, blue) }')
  ...
  def test_background_repeating_linear_gradient
    ...
    decls = parse_and_flatten('.test { background: repeating-linear-gradient(45deg, red, blue 10px) }')
  ...
  def test_background_repeating_radial_gradient
    ...
    decls = parse_and_flatten('.test { background: repeating-radial-gradient(circle, red, blue 10px) }')
  ...
  def test_background_conic_gradient
    ...
    decls = parse_and_flatten('.test { background: conic-gradient(red, yellow, green) }')
  ...
  def test_different_selectors_separate
    ...
    id_rule = flattened.rules.find { |r| r.selector == '#id' }
  ...
  def test_calc_values
    decls = parse_and_flatten('.test { margin: 10px calc(100% - 20px) }')
