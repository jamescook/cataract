...
module Cataract
  ...
  class NewStylesheet
    ...
    def initialize
      @rules = [] # Flat array of NewRule structs
      @media_index = {} # Hash: Symbol => Array of rule IDs
      @charset = nil
    end

    # Parse CSS and return a new NewStylesheet
    #
    # @param css [String] CSS string to parse
    # @return [NewStylesheet] Parsed stylesheet
    def self.parse(css)
      new.add_block(css)
    end
    ...
    # @return [Array<NewRule>] Matching rules
    ...
    #
    # @return [Array<NewRule>] Rules with no media query
    def base_rules
    ...
    def to_s(which_media = :all)
      ...
      if which_media_array.include?(:all)
        Cataract._stylesheet_to_s_new(@rules, @media_index, @charset)
      else
        ...
        matching_rule_ids.sort.each do |old_id|
          ...
          rule = @rules[old_id]
          filtered_rules << Cataract::NewRule.new(new_id, rule.selector, rule.declarations, rule.specificity)
        ...
        Cataract._stylesheet_to_s_new(filtered_rules, filtered_media_index, @charset)
      end
    end
    alias to_css to_s
    ...
    # Load CSS from file
    #
    # @param path [String] Path to CSS file
    # @return [NewStylesheet] Parsed stylesheet
    ...
    def add_block(css, fix_braces: false, media_types: nil)
      ...
      result = Cataract.parse_css_new(css)
    ...
    # @return [Array<NewRule>] Array of matching rules
    ...
    # @yieldparam rule [NewRule] The matching rule object
    ...
    def inspect
      total_rules = size
      if total_rules.zero?
        '#<Cataract::NewStylesheet empty>'
      else
        preview = @rules.first(3).map(&:selector).join(', ')
        more = total_rules > 3 ? ', ...' : ''
        "#<Cataract::NewStylesheet #{total_rules} rules: #{preview}#{more}>"
      end
    end
    ...
    #
    # @return [NewStylesheet] New stylesheet with a single merged rule
    def merge
      # C function handles everything - returns new NewStylesheet
      Cataract.merge_new(self)
    end
    ...
    #
    # @param rule [NewRule] Rule to check (AtRule filtered out by each_selector)
